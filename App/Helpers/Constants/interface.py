GAME_TITLE = "Supermarket"

#SCREEN
SCREEN_WIDTH = 832
SCREEN_HEIGHT = 624

#IMG
PRODUCT_IMAGE_PATH = "Images/Products/"
CUSTOMER_IMAGE_PATH = "Images/Customers/"

#COLORS:
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)

RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0, 0, 255)

GRAY = (200, 200, 200)
DARK_GRAY = (75, 75, 75)

RED_GRAY = (255, 150, 150)

#MAP
MAP_OFFSET = 50

#GRID
GRID_CELL_SIZE = 26

#CUSTOMER
CUSTOMER_COLOR = RED_GRAY
CUSTOMER_WIDTH = GRID_CELL_SIZE
CUSTOMER_HEIGHT = GRID_CELL_SIZE

#PLAYER
PLAYER_WIDTH = GRID_CELL_SIZE
PLAYER_HEIGHT = 2*GRID_CELL_SIZE

#TABLET
TABLET_OFFSET = 100
TABLET_WIDTH = SCREEN_WIDTH - 2*TABLET_OFFSET
TABLET_HEIGHT = SCREEN_HEIGHT - 2*TABLET_OFFSET

#PRODUCT IN TABLET
PRODUCT_OFFSET = 10
PRODUCT_WIDTH = (TABLET_WIDTH - (6*PRODUCT_OFFSET) ) / 3
PRODUCT_HEIGHT = (TABLET_HEIGHT - (6*PRODUCT_OFFSET) ) / 3

#BUTTON
BUTTON_WIDTH = 150
BUTTON_HEIGHT = 100
BUTTON_Y = TABLET_OFFSET + TABLET_HEIGHT/2 - BUTTON_HEIGHT/2
BUTTON_OFFSET = (TABLET_WIDTH - 3*BUTTON_WIDTH) / 4

#EXIT
EXIT_BUTTON_WIDTH = 36
EXIT_BUTTON_HEIGHT = 36

#SHELF
SHELF_HEIGHT = 2*GRID_CELL_SIZE
SHELF_WIDTH = 4*GRID_CELL_SIZE

#CASH REGISTER
CASH_REGISTER_X = 500
CASH_REGISTER_Y = 400
CASH_REGISTER_WIDTH = 100
CASH_REGISTER_HEIGHT = 100

#BOX
BOX_WIDTH = 50
BOX_HEIGHT = 50
BOX_COLOR = GREEN

#ADD SHELF BUTTON
ADD_SHELF_BUTTON_X = SCREEN_WIDTH - 200 
ADD_SHELF_BUTTON_Y = SCREEN_HEIGHT - 200
ADD_SHELF_BUTTON_WIDTH = 130
ADD_SHELF_BUTTON_HEIGHT = 40

#MENU BUTTONS
MENU_BUTTON_WIDTH = 350
MENU_BUTTON_HEIGHT = 70
MENU_BUTTON_X = (SCREEN_WIDTH - MENU_BUTTON_WIDTH) / 2
MENU_BUTTON_OFFSET = 30
MENU_BUTTON_Y = (SCREEN_HEIGHT - 4*MENU_BUTTON_HEIGHT - 3*MENU_BUTTON_OFFSET) / 2

#TEMP MONEY MESSAGE
TEMP_MONEY_MESSAGE_X = 10
TEMP_MONEY_MESSAGE_Y = 85

#LEAVING POSITION
LEAVING_X = 0
LEAVING_Y = SCREEN_HEIGHT - 100

#TEXT
FONT_SIZE = 36
TEXT_OFFSET = 6
